...
1 goroutine采用动态栈.线程采用的是固定大小的栈(一般2M);协程动态栈初始化时只需要2KB,等到具体使用时容量可以动态伸缩.    
2 goroutine自己实现调度器.线程是操作系统内核调度;内核调度流程:硬件计时器中断处理器->调用内核函数scheduler->挂起当前执行线程,将寄存器内容读入内存->选择下一个线程并恢复寄存器信息->线程执行;这个流程就是线程的上下文切换流程;goroutine中通过设计语句来进行调度(如休眠,channel调用,同步阻塞时,进行协程切换执行);goroutine不会进去内核上下文,调度效率更高;    
3 goroutine通过GOMAXPROCS变量控制线程数量.线程是根据内核数量生成;    
4 goroutine没有id号,不支持goroutine显示获取;线程可以通过其id号,从线程本地存储获取.
